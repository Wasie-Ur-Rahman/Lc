...
const NC_Field_Count = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.post('https://192.168.18.251:8010/Stats_getLC_NC_FIELDS_COUNT/');
    ...
    return (
        <div style={{ width: '60%', height: '500px' }} className='' >
            {loading ? <p>Loading...</p> : error ? <p>Error: {error.message}</p> : <Bar data={chartData} options={options} />}
        </div>
    );
};
