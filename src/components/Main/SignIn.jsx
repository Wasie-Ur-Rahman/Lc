...
const SignIn = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (error.response) {
        setError(
          `Authentication failed. Status code: ${error.response.status}`
        );
  ...
  return (
    <div className="flex flex-col justify-evenly w-[30%] h-[100%] bg-[#2C333D] opacity-[84%] border-[1px] border-[#242F3A]">
      <form
        ...
        className="h-[82%] flex flex-col justify-evenly"
      >
      ...
      {isWaiting && (
        <p className="text-[#2A8BE9] mt-2 text-[18px] text-center">
          {" "}
          Signing in
      ...
      <div className="flex h-[18%] justify-center mt-5">
        ...
        <a className="text-blue-500 hover:underline ml-2 font-Inconsolata text-[22px] cursor-pointer">
          Email/Password?
        </a>
      </div>
